OC.L10N.register(
    ...
    "Workflow OCR Backend is not reachable. Error was: %1$s" : "Podpůrná vrstva pro Workflow OCR není dosažitelná. Chyba byla: %1$s",
    ...
    "Failed to save settings: {error}" : "Nastavení se nepodařilo uložit: {error}",
    "Failed to load global settings: {error}" : "Nepodařilo se načíst globální nastavení: {error}",
    "Select language(s)" : "Vybrat jazyk(y)",
    "Failed to load installed OCR languages: {error}" : "Nepodařilo se načíst nainstalované jazyky pro OCR: {error}",
    ...
    "The asynchronous OCR process will send Nextcloud notifications. Use these settings to control them." : "Proces asynchronního OCR bude posílat Nextcloud notifikace. Pomocí těchto nastavení je ovládejte.",
    "Skip notifications for ocrmypdf error code 2 (for example digitally signed PDFs)" : "Přeskočit notifikace pro kód chyby 2 v ocrmypdf (například digitálně podepsané PDF dokumenty)",
    "Skip for invalid PDFs" : "Přeskočit neplatné PDF dokumenty",
    "Skip notifications for ocrmypdf error code 8 (for example password protected PDFs)" : "Přeskočit notifikace pro kód chyby 8 v ocrmypdf (například heslem chráněné PDF dokumenty)",
    "Skip for encrypted PDFs" : "Přeskočit šifrované PDF dokumenty",
    "Send a notification when OCR processing completes successfully." : "Když OCR zpracovávání bude úspěšně dokončeno, poslat notifikaci.",
    ...
    "Keep the original file as a version labeled Before OC and exclude it from automatic expiration." : "Ponechat původní soubor jako verzi označenou jako Před OCR a vynechat ji z automatického skončení platnosti.",
    "Keep original file version" : "Ponechat původní verzi souboru",
    "Restore the original modification date on the new file version for consistent sorting." : "Kvůli konzistentnímu řazení, obnovit původní datum změny u nové verze souboru.",
