...
Custom ocrMyPdf CLI arguments | If you want to pass custom arguments to the `ocrmypdf` CLI, you can do so here. Please note that the arguments will be passed as they are to the CLI, so make sure to use the correct syntax. Check the [official docs](https://ocrmypdf.readthedocs.io/en/latest/cookbook.html) for more information.<br/>:information_source: The value is split into single arguments (values containing spaces can be wrapped in single or double quotes, e.g. `--title "My Document"`) and each of them is passed to `ocrmypdf` as a literal argument. There is **no shell expansion**, so constructs like `$(...)`, backticks, `\|` or `>` are treated as plain text. |
