	...
	public function runOcrProcess(int $fileId, string $uid, WorkflowSettings $settings) : void { // TODO :: make private
		try {
			...
			if ($exitCode === 6) {
				...
				throw new OcrNotPossibleException('File appears to contain text so it may not need OCR. Message: ' . $result->getErrorMessage());
			}
			...
			if ($exitCode === 2 && $settings->getSkipOcrErrors()) {
