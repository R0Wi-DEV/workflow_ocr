...
class OcrService implements IOcrService {
	...
	private function doPostProcessing(Node $file, string $uid, WorkflowSettings $settings, OcrProcessorResult $result, ?int $fileMtime = null): void {
		...
		if ($result->getRecognizedText() !== '') {
			...
			if ($settings->getCreateSidecarFile()) {
				$this->createSidecarTextFile($file, $result->getRecognizedText(), $newFilePath);
			}
		}
	 ...
	 */
	private function createSidecarTextFile(Node $originalFile, string $recognizedText, string $newFilePath): void {
