...
class AppApiWrapper implements IAppApiWrapper {
	...
	public function exAppRequest(
		...
		$appApiFunctions = $this->container->get(PublicFunctions::class);
		return $appApiFunctions->exAppRequest(
			$appId,
			$route,
			$userId,
			$method,
	...
	public function getExApp(string $appName): ?array {
		$appApiFunctions = $this->container->get(PublicFunctions::class);
		return $appApiFunctions->getExApp($appName);
	}
}
